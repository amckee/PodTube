...
class ChannelHandler(web.RequestHandler):
    ...
    def get_html( self, channel ):
        ...
        if debug:
            ...
            with webdriver.Firefox( options=self.set_options() ) as driver:
                ...
                driver.get( url )
                time.sleep( 5 )
                el = driver.find_element_by_class_name( "claim-grid" )
                html = el.text
        return html
    ...
    def generate_rss( self, url ):
        ...
        ## since we got this far, let's create the feedgen object and fill with channel details
        feed = FeedGenerator()
        ...
        feed.description( el.find("p", "name").text )
